...
void base_cache::initialize()
{
	...
	// ADDED
	else if (decision_policy.find("ffc")==0)			// FFC: set the size of the name cache
	{
		name_cache_size = par("NC");
		if (decision_policy.compare("ffc")==0) {
			decisor = new FFC(name_cache_size);
		} else {
			target_acceptance_ratio_string = decision_policy.substr(3);
			target_acceptance_ratio = atof( target_acceptance_ratio_string.c_str() );
			decisor = new FFC(target_acceptance_ratio, name_cache_size);
		}
	}
	else if (decision_policy.compare("tfc")==0)
	{
		name_cache_size = par("NC");
		decisor = new FFC(name_cache_size, true);
	}
	else if (decision_policy.find("wfc")==0)
	{
		name_cache_size = par("NC");	
		target_acceptance_ratio_string = decision_policy.substr(3);
		target_acceptance_ratio = atof( target_acceptance_ratio_string.c_str() );
		decisor = new FFC(name_cache_size, true, target_acceptance_ratio);

	}
/*
	else if (decision_policy.find("wfc")==0)			// WFC: set the size of the name cache
	{
		name_cache_size = par("NC");
		decisor = new WFC(target_acceptance_ratio, name_cache_size);
	}
*/
	...
	// INPUT_CHECK
	if ( decision_policy.find("fix")==0 || decision_policy.find("wfc")==0 || (decision_policy.find("ffc")==0 && !decision_policy.compare("ffc") == 0) || (decision_policy.find("costaware")==0 && !decision_policy.find("ideal_costaware")== 0))
	{
		if ( strlen( target_acceptance_ratio_string.c_str() ) == 0 )
		{
			std::stringstream ermsg;
			ermsg<<"You forgot to insert a valid value of acceptance rate when "<<
				"specifying the decision policy. Right examples are fix0.01, ffc0.01, costaware0.1";
			severe_error(__FILE__,__LINE__,ermsg.str().c_str() );
		}
...
void base_cache::store(cMessage *in)
{
    ...
    if (decisor->data_to_cache((ccn_data*)in )) 	// The decision is based on the meta-caching strategy.
    {
		decision_yes++;
		#ifdef STREAM_DEBUG
			cout << "NODE # " << getParentModule()->getIndex() << " :caching CONTENT # " << ( (ccn_data* ) in )->getChunk() << endl;
		#endif
		data_store( ( (ccn_data* ) in )->getChunk() ); // Store the received chunk inside the local cache. It is implemented 											   // by each derived class according to the chosen replacement policy.
...
void base_cache::store_name(chunk_t elem)
{
    ...
    #ifdef STREAM_DEBUG
	cout << "NODE # " << getParentModule()->getParentModule()->getIndex() << " :caching NAME # " << elem << endl;
    #endif
