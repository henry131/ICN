...
void statistics::initialize(int stage)
{
	if(stage == 1)
	{
		...
		partial_n = partial_n*(double)num_nodes;
		partial_n = round(partial_n);

		#ifdef SEVERE_DEBUG
			cout << "PARTIAL N Nodes: " << partial_n << endl;	
		#endif
		...
		string cs = caches[0]->par("DS");

		//Override parital_n if never caching to avoid prolonged stabilization phase
		if (cs.compare("never") == 0){
			partial_n = 0;
		}

		#ifdef SEVERE_DEBUG
			string fs = caches[0]->getParentModule()->par("FS");
			cout << "Forwarding Strategy:\t" << fs << endl;
			cout << "Caching Strategy:\t" << cs << endl;
...
void statistics::handleMessage(cMessage *in)
{
    ...
    case FULL_CHECK:
        ...
        if (full >= partial_n || simTime() >= 3*360)
        //if (full >= partial_n)
        ...
        //if (simTime()>=10*3600)
        {
        	...
        	cout<< full << " caches filled at time "<< simTime() << endl;
    	...
    	if ( stables >= partial_n )
    	{
    		...
    		if(stable_with_traffic >= floor(partial_n/2))
    		{
				for (int i = 0;i<num_nodes;i++)
				{
					...
					// In case of FFC					
					//FFC* ffcDecisor = dynamic_cast<FFC *> (decisor);
					//if(ffcDecisor)
					//	ffcDecisor->set_stable();
					// In case of WFC					
					//WFC* wfcDecisor = dynamic_cast<WFC *> (decisor);
					//if(wfcDecisor)
					//	wfcDecisor->nc_stable = true;
...
bool statistics::stable(int n)
{
    ...
    if ( fabs( samples[n].size() - window * 1./ts ) <= 0.001 )	// Calculate the variance each window samples.
	{
    	...
    	//if ( sqrt(var) <= 0.005 * mean)
        {
            ...
            stable = true;
            cout << "NODE # " << n << " is STABLE at # " << simTime() << endl;
	    //cout << " with Samples:" << endl;
...
void statistics::finish()
{
    ...
    double i_h = (double) (global_tot_downloads - global_repo_load) / global_tot_downloads;
    recordScalar(name , i_h) ;
    cout << "inner_hit: " << i_h << endl; 
    #ifdef SEVERE_DEBUG
