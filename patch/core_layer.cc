...
void core_layer::handle_interest(ccn_interest *int_msg)
{
	...
	#ifdef STREAM_DEBUG
		cout << "NODE # " << getIndex() << " : receiving interest for Content # " << chunk << endl;
	#endif
    ...
    if (decision_policy.compare("two_lru")==0)		// 2-LRU
    {
    	...
    	cacheable = tLruPointer->name_to_cache(chunk);
    }
	...
	//update it's location in name-cache (re LRU)
	if (decision_policy.find("ffc") || decision_policy.find("tfc") || decision_policy.find("wfc"))
	{
		//cacheable not used - dummy variable for bool receiver
		//update done in lookup function
    		FFC* ffcPointer = dynamic_cast<FFC *> (ContentStore->get_decisor());
		cacheable = ffcPointer->lookup_name_cache(chunk);
	}	
	...
	#ifdef STREAM_DEBUG
		cout << "NODE # " << getIndex() << " : satisfying interest for Content # " << chunk << endl;
	#endif
